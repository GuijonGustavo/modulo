...
{	//<<<<<<<<<<<<<<<---- FUNCION QUE AGREGA FILA---- >>>>>>>>>>>>>
	...
	$(document).on('click','.clsEliminarFila',function()
	{
		...
		//<<<<<<<<<<<<<<<---- FUNCION QUE ELIMINA FILA Calidad de los datos---- >>>>>>>>>>>>>
	$(document).on('click','.clsEliminarFila_d',function()
	{
		var objCuerpo_d=$(this).parents().get(2);
		if($(objCuerpo_d).find('tr').length==2)
		{
			return;
		}
		var objFila_d=$(this).parents().get(1);
		$(objFila_d).find('tr').remove();
	});
    ...
    $(document).on('click','.clsAgregarFilad',function()
	{
   		var objCuerpo=$(this).parents().get(2);
		var objTabla=$(this).parents().get(3);
		var nodef= objTabla.childNodes[1].firstChild.cloneNode(true);
		var nodel= objTabla.childNodes[1].lastChild.cloneNode(true);
 	    var strNueva_Fila='<tr>'+nodef.innerHTML+'</tr><tr>'+nodel.innerHTML+'</tr>';
