...
$(document).ready(function(){
    ...
    $('#c_escala').click(function(){
                $("#c_escala").val(function(){$this.append("<select>lput</select>")});
                    });
	...
	var paisOtro= $('select[name = pais]').val();
...
//////////////////////////PaisOtro
	...
	if (paisOtro != '3')
	{
	    if (estado.val() == '')
	    {
		estado.addClass('highlight');
		$('.error').replaceWith('<div class="error">Ingrese el Estado</div>');
		$('.error').fadeIn('slow');
		$(".error").fadeOut(10000);
		$(estado).focus();
		return false;
    ...
    });// FIN $('#atributos').click(function ()



//Comienza validar RegistroColaborador



    $('#RegistroColaborador').click(function (){

	var nombreCapturista = $('input[name = nombreCapturista]');
	var userCapturista = $('input[name = userCapturista]');
	var passCapturista = $('input[name = passCapturista]');
	var correoCapturista = $('input[name = correoCapturista]');

	var validaColaborador = [ [nombreCapturista, " Ingrese el nombre del colaborador"] ,
				 [userCapturista , " Ingrese el usuario del colaborador"],
				 [passCapturista , " Ingrese el password del colaborador"],
				 [correoCapturista , " Ingrese el correo del colaborador"]
			       ];

	for (i = 0 ; i < validaColaborador.length ; i++)
	{
	    if (typeof(validaColaborador[i]) !== 'undefined')
	    {
		if (validaColaborador[i][0].val() == '')
		{
		    validaColaborador[i][0].addClass('highlight');
		    $('.error').replaceWith('<div class="error">' + validaColaborador[i][1] + '</div>');
		    $('.error').fadeIn('slow');
		    $(".error").fadeOut(10000);
		    $(validaColaborador[i][0]).focus();
		    return false;
		}
		else
		{
		    validaColaborador[i][0].removeClass('highlight');
		    $('.error').hide();
		}
	    }
	} // FIN FOR
    });// FIN $('#RegistroColaborador').click(function ()


///////////////////////Termina validar RegistroColaborador

    var val_pais = document.getElementById("pais").value ;

    if (val_pais == 3) { $("#OTRO").show(); }
    
    var val_edo = document.getElementById("estado").value ;

    if (val_edo) { document.getElementById("estado").disabled = false; }
...
function cambiopais(str)
{
    ...
    if ( str != "3" && str != "")
    {
	loadDoc("pais="+str,"PHP/estado.php", function()
		{
...
$(function()
  {
	  ...
	  buttons:
	  {
	      "BuscarPrueba": function()
	      {

		  // creating input on-the-fly
/*		  var form = $(document.createElement('form'));
form.attr("action","upload_zipear.php");
form.attr("id","xxx");
...
*/
		  ...
		  //		  input.attr("id","hola");
		  window["form"]=form;//para convertir la variable en global
		  window["input"]=input;//para convertir la variable en global		  
		  form.submit();		  
		  input.trigger('click'); // opening dialog
		  alert("Aquí va el input:"  + input);
		  alert("Aquí va el id:"  + form.attr("id","hola"));		  		  
...
//				      var fileName = e.type.files[0].name;				      
				      alert(fileName);
				      
				      window["fileName"]=fileName;//para convertir la variable en global
//				      window["input"]=input;
				      //				      window.location.href = "upload_zipear.php?archivo_zipear=" + fileName;
				      //				      var fileN = "<?php echo $fileName;?>";
...
//		  });
		  var other_data = $('#hola').serializeArray();
		  alert("Aquí el other_data:" + other_data);
		  $.each(other_data,function(key,input){
		      data.append(input.name,input.value);
		  });
//		  jQuery.ajax({
		  $.ajax({		      
		      url: 'upload_zipear.php',
		      data: other_data,
		      cache: false,
		      contentType: false,
		      processData: false,
			  ...
			  alert("Aquí va el data: " + other_data);
		      }
		  });
      ...
      $( "#borrar" ).click(function() {$( "#dialog_borrar" ).dialog( "open" );});
