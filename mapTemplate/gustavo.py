...
conn = pg.connect(dbname='dist_jm071', user='postgres', passwd='sig123456', host='200.12.166.29')

###########Aqui va la lista de shapes#################3


lista_shapes = os.listdir(r'T:\\jm\\jm071\\dist\\shp')
lista_shapes.sort()

for i,j in enumerate(lista_shapes):
    print i,j

for shapefile in lista_shapes[3:]: #si se intenta un shape en particular se anade un rango lista_shapes[45:46]
...
#Cita para sitios de recolecta
        ...
        cita1 = cita+". "+fecha[2]+". "+cita_shape+", "   
        cita2 = "escala "+escala_query+". "+publish_query+", "+pubplace_query+"."
        cita1+=cita2
    ...
    newlayer1 = arcpy.mapping.Layer(r'T:\\jm\\jm071\\dist\\'+shapefile+'')




    desc = arcpy.Describe(r'T:\\jm\\jm071\\dist\\'+shapefile+'')
